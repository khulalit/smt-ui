...
export const EmployeeTable: React.FC<EmployeeTableProps> = ({
  ...
  const getDeptBadgeClass = (dept: string) => {
    switch (dept) {
      ...
      case 'Product': return 'bg-indigo-50 text-indigo-700 border-indigo-100';
      case 'Design': return 'bg-cyan-50 text-cyan-700 border-cyan-100';
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-slate-100 text-left">
          <thead className="bg-slate-50">
            <tr>
              <th scope="col" className="px-6 py-3 text-xs font-semibold text-slate-600">Employee</th>
              <th scope="col" className="px-6 py-3 text-xs font-semibold text-slate-600">Job Details</th>
              <th scope="col" className="px-6 py-3 text-xs font-semibold text-slate-600">Department</th>
              <th scope="col" className="px-6 py-3 text-xs font-semibold text-slate-600">Region</th>
              <th scope="col" className="px-6 py-3 text-xs font-semibold text-slate-600">Salary</th>
            ...
            {employees.length > 0 ? (
              employees.map((emp) => (
                <tr key={emp.id} className="hover:bg-slate-50 transition-colors duration-150">
                  ...
                  {/* Job Title / Hire Date */}
                  <td className="whitespace-nowrap px-6 py-4">
                    <div className="text-sm font-medium text-slate-800">{emp.jobTitle}</div>
                  ...
                  {/* Department Badge */}
                  <td className="whitespace-nowrap px-6 py-4">
                    <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium ${getDeptBadgeClass(emp.department)}`}>
                      {emp.department}
                    </span>
                  </td>
                  {/* Country */}
              ...
              <tr>
                <td colSpan={6} className="px-6 py-12 text-center">
                  <div className="text-sm font-medium text-slate-400">No employees found matching the filters.</div>
                </td>
              </tr>
            )}
          </tbody>
