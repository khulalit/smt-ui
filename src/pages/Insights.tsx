...
export const InsightsDashboard: React.FC = () => {
  ...
  const totalEmployees = insights.global?.totalCount ?? 0;
  const totalPayroll = insights.global?.totalPayroll ?? 0;
  const averageGlobalSalary = insights.global?.avgSalary ?? 0;

  // Map API response to component-friendly structures
  const countryMetrics =
    insights.byCountry?.map((c: CountryMetric) => ({
      country: c.country,
  ...
  const activeCountryMetric = countryMetrics.find(
  ...
  const jobMetrics =
    insights.byJobTitle?.map((title: JobTitleMetric) => ({
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1.5 text-sm text-slate-500">
            Payroll analytics, country salary distributions, and role
            benchmarking.
          </p>
        </div>
      ...
      <div className="mt-8 rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
        <h3 className="mb-6 text-lg font-bold text-slate-900">
          Global Regions Comparison Matrix
        </h3>
              ...
              {countryMetrics.map((metric) => (
                <tr
                  ...
                  className={`hover:bg-slate-50 transition-colors duration-150 cursor-pointer $${
                    metric.country === selectedCountry ? "bg-indigo-50/30" : ""
                  }`}
